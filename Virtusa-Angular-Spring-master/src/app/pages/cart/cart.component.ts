...
val=false;
    ngOnInit() {
       ...
       {
           this.val=true;
        this.productListData=(this.cartService.productList)
       }
        //console.log("Wel  ",localCart);
        this.cartService.getCart().subscribe(prods => {
            console.log("welcome",prods);
...
myval=0;
    ...
    payment()
    {
        if(this.cartService.payment(this.productListData))
        {
            alert("Payment Done")
        }
    ...
    ngAfterContentChecked() {
        this.total = this.productInOrders.reduce(
          ...
          // if(this.val)
            this.myval= this.total = this.productInOrders.reduce(
                (prev, cur) => prev + cur.count * cur.productPrice, 0);
    }
    ...
    checkout() {
        if (!this.currentUser) {
            ...
            this.cartService.checkout().subscribe(
                _ => {
                    this.productInOrders.push(this.productInOrders);
                },
                error1 => {
